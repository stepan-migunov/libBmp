...
int openImage(char* fileName)
{
    ...
    imageName = malloc(fileNameLen+1);
    ...
    bitmap = malloc(x_size * sizeof(struct ColorBit*));
    for(size_t i = 0; i < x_size; ++i)
        bitmap[i] = malloc(y_size * sizeof(struct ColorBit));

    b0 = fgetc(file), b1 = fgetc(file);
    BMPHeader.bitsPerPixel = b0 | (b1<<8u);

    BMPHeader.idunnotheirmeanings = malloc(BMPHeader.hatSize - 16);
...
int create(char* fileName, size_t width, size_t height)
{
    if(bitmap != NULL)
        return -1;
    ...
    BMPHeader.idunnotheirmeanings = malloc(24);
    for(int i = 0; i < 24; ++i)
...
void drawLine_point(struct point begin, struct point end, int depth, struct ColorBit color)
{
    ...
    for(size_t x = min_t(begin.x, end.x); x <= max_t(begin.x, end.x); ++x)
        for(size_t y = min_t(begin.y, end.y); y <= max_t(begin.y, end.y); ++y)
        {
            ...
            if (temp < 0.5)
                drawCircle_point((point) {x, y}, depth / 2, color);
        }
}
